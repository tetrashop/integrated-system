...
// نمایش پروژه‌ها در جدول
function renderProjects() {
    const tbody = document.getElementById('projectsBody');
    if (!projects || projects.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="color:#a7a9be;padding:20px;">هیچ پروژه‌ای یافت نشد</td></tr>';
        return;
    }
...
// دریافت آنالیز و به‌روزرسانی کارت‌ها
async function updateAnalytics() {
    try {
        ...
        statusSummary.innerHTML = Object.entries(data.status_counts || {}).map(([k,v]) => 
            `<div>${k}: ${v}</div>`
        ).join('');
    } catch (error) {
...
function editProject(id) {
    ...
    if (!project) return;
    // می‌توانید یک فرم ویرایش ساده اضافه کنید، فعلاً یک alert ساده
    alert(`ویرایش پروژه: ${project.name}\nپیشرفت: ${project.progress}%\nوضعیت: ${project.status}`);
    // برای ویرایش کامل، می‌توانید یک مودال مشابه مودال افزودن بسازید.
}
